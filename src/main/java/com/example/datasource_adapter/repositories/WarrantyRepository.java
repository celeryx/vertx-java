...
import static io.vertx.sqlclient.templates.TupleMapper.mapper;
...
public class WarrantyRepository extends BaseRepository<WarrantyEntity> {
  ...
  private static final RowMapper<String> ID_MAPPER = RowMapper.newInstance(row -> row.getString("id"));

  @Inject
  public WarrantyRepository(Pool dbPool) {
    super(dbPool, mapper(WarrantyEntity::toPersistenceMap), row -> row.toJson().mapTo(WarrantyEntity.class));
  }
